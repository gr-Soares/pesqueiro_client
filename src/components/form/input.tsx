...
export const InputSelect = React.forwardRef((
    { register, options, name, title, onSelect }:
        { register: any, options: string[], name: string, title: string, onSelect?: (value: string) => void}, ref
) => {
    return (
        <div>
            <label className="text-sm text-gray-700 block mb-1 font-bold">{title}</label>
            <select {...register(name)} name={name} onChange={(e) => onSelect ? onSelect(e.target.value) : {}} id={name} defaultValue={options[0]} className="bg-gray-100 border border-gray-200 rounded py-1 px-3 block focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full">
                {options.map((value) => (
